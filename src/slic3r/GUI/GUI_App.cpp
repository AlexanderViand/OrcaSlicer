...
// FULU: build the X-BBL-* HTTP headers used to identify the slicer to Bambu's cloud.
// On Linux (or when the Linux bridge is forcing a BambuStudio identity), we masquerade
// as BambuStudio with a pinned client version so the BBL API accepts us.
            ...
            if (!ret) {
                if (auto iter = values_map.find(BBL_JSON_KEY_BASE_ID); iter != values_map.end() && iter->second == setting_id) {
                    ...
                    if (http_code >= 400) {
                        ...
                        // FULU: accept the BBL legacy key (IOT_*) too, fall back to Orca's key.
                        auto it = values_map.find(IOT_JSON_KEY_UPDATE_TIME);
                        if (it == values_map.end() || it->second.empty())
                            it = values_map.find(ORCA_JSON_KEY_UPDATE_TIME);
                        if (it != values_map.end() && !it->second.empty())
