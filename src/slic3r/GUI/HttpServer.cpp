...
std::shared_ptr<HttpServer::Response> HttpServer::auth_handle_request(const std::string& url, const std::string& provider)
{
    ...
    if (boost::contains(url, "access_token")) {
        ...
        // FULU: surface get_my_profile errors back to the caller instead of falling through.
    ...
    // (FULU's ticket-based flow is handled at the top of this function via TicketLoginTask /
    // build_canonical_login_payload; upstream's inline ticket flow was a duplicate, removed.)
