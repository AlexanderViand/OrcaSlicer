...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    // X2D / FS01 wipe-tower port — bambu/master config keys consumed by
    // the new WipeTower.cpp architecture. Default values are set in
    // PrintConfig.cpp. See the matching block there for rationale.
    ...
    ((ConfigOptionFloats,              filament_ramming_travel_time))
    ((ConfigOptionFloats,              filament_ramming_travel_time_nc))
    ((ConfigOptionFloats,              filament_ramming_volumetric_speed))
    ((ConfigOptionFloats,              filament_ramming_volumetric_speed_nc))
